...
class ExpressAdapterHandler extends AdapterHandler {
  ...
  private async setupRedocHtml(): Promise<void> {
    ...
    const redocHTML = await renderRedocView({
      path,
    ...
    this.httpAdapter.get(this.path, async (req: Request, res: Response) =>
      res.send(redocHTML)
    );
  }
...
class FastifyAdapterHandler extends ExpressAdapterHandler {
  ...
  private setupJSForFastify(): void {
    const pathToModule = require.resolve("redoc-try-it-out");
    this.httpAdapter.get(
      `${this.path}/${tryItOutJsMinFileName}`,
  ...
  private async setupRedocHtmlForFastify(): Promise<void> {
    const { path, docUrl, options, document } = this;
    const redocHTML = await renderRedocView({
      path,
      tryItOutJsMinFileName,
    ...
    this.httpAdapter.get(
      this.path,
      async (req: FastifyRequest, res: FastifyReply) => res.send(redocHTML)
    );
  }

  private setupDocUrlForFastify(): void {
    this.httpAdapter.get(
      this.docUrl,
      (req: FastifyRequest, res: FastifyReply) => {
  ...
  protected async setup(): Promise<void> {
    await this.setupRedocHtmlForFastify();
    this.setupDocUrlForFastify();
    this.setupJSForFastify();
  }
}
